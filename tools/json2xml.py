...
# 1.标签路径
labelme_path = r"./origin"  # 原始json、bmp标注数据路径,需要更换成自己的数据集名称
saved_path = r"./annotations"  # 保存路径
...
for json_filename in files:
     ...
     with codecs.open(xmlName, "w", "utf-8") as xml:
        ...
        # -------------------------------------------------
        xml.write('\t<source>\n')
        xml.write('\t\t<database>shoes</database>\n')
        ...
        xml.write('</annotation>')
        # 复制图片
        newImgName = os.path.join('./images', img_name.split("/")[-1])
